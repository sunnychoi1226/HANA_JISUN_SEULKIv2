...
function toDb(q,user){
  return {
    ...
    nights:+q.nights||0,
    room_types:Array.isArray(q.roomTypes)?q.roomTypes.slice(0,5):[],
    r1:+(q.roomTypes?.[0]?.count)||+q.r1||0,rate1:+(q.roomTypes?.[0]?.rate)||+q.rate1||0,
    r2:+(q.roomTypes?.[1]?.count)||+q.r2||0,rate2:+(q.roomTypes?.[1]?.rate)||+q.rate2||0,
    guests:+q.guests||0,
    ...
    tax_incl:!!q.taxIncl,fee_taxed:!!q.feeTaxed,bf_incl:!!q.bfIncl,
    quote_terms:q.quoteTerms&&typeof q.quoteTerms==='object'?q.quoteTerms:{},
    source:q.source||'manual',
    updated_by:user.employee_id,updated_at:new Date().toISOString()
  };
}
function fromDb(x){
  const roomTypes=Array.isArray(x.room_types)&&x.room_types.length?x.room_types:[
    ...(Number(x.r1)>0?[{name:'객실타입1',count:Number(x.r1),rate:Number(x.rate1)||0}]:[]),
    ...(Number(x.r2)>0?[{name:'객실타입2',count:Number(x.r2),rate:Number(x.rate2)||0}]:[])
  ];
  return {id:x.id,event_name:x.event_name,status:x.status,hotel:x.hotel,city:x.city,cur:x.cur,nights:x.nights,roomTypes,r1:x.r1,rate1:x.rate1,r2:x.r2,rate2:x.rate2,guests:x.guests,
    ...
    taxIncl:x.tax_incl,feeTaxed:x.fee_taxed,bfIncl:x.bf_incl,
    quoteTerms:x.quote_terms||{confirmed:[],conditional:[],review:[]},
    source:x.source,created_by:x.created_by,updated_by:x.updated_by,created_at:x.created_at,updated_at:x.updated_at};
}
