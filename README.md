...
이번 버전은 인증을 두 단계로 운영할 수 있습니다.

### 개발/테스트 단계 — 기본값
Netlify에 `AUTH_MODE`를 설정하지 않거나 `test`로 설정하면:
- `H`로 시작
- 총 5자리
- 영문/숫자 조합 허용
...
이 단계에서도 각 API 요청은 입력한 사번을 `X-Employee-Id`로 전달하므로,
견적 데이터의 `created_by`, `updated_by` 등에는 해당 테스트 사번이 남습니다.

### 실제 운영 단계
Netlify Environment variables에 아래를 추가:

AUTH_MODE=production

그러면 같은 코드가 자동으로 Supabase `users` 테이블을 확인하고,
`active=true`인 등록 사번만 통과시킵니다.

즉 운영 전환 시 코드 수정은 필요 없고 환경변수만 변경하면 됩니다.

### 중요
테스트 모드는 실제 신원 인증이 아닙니다.
실제 부서 운영 시에는 `AUTH_MODE=production` 또는 Microsoft Entra ID / 사내 SSO 적용을 권장합니다.


## V24 지역/도시 검색 변경
- 권역은 6개로 고정: 동남아 / 일본 / 미주 / 남태평양 / 유럽 / 중화권
- 하와이, 호주, 뉴질랜드는 남태평양으로 분류
- 도시는 드롭다운이 아니라 자유 입력 + datalist 추천 방식
- Supabase Master에 없는 도시도 입력 가능
- 기존 도시 메타가 없으면 Netlify Function이 OpenStreetMap Nominatim으로 도시 좌표를 확인
- 좌표 확인 후 Overpass에서 해당 도시 주변 신규 호텔 후보를 검색
- 신규 도시의 통화/세금/항공편 정보는 임의 생성하지 않고 '확인 필요'로 표시
- `APP_CONTACT_EMAIL` 환경변수를 선택적으로 설정하면 Nominatim 요청 User-Agent에 연락처를 넣을 수 있음


## V26 Cache-first 구조
- 주요도시 39개는 Supabase `cities`에 사전 등록
- 도시 입력 시 `cities`에서 좌표를 먼저 읽어 지도 중심 즉시 표시
- 호텔 후보는 `hotel_discovery_cache` 30일 캐시 우선
- 캐시가 없을 때만 Overpass 신규 검색
- 외부 후보는 도시당 최대 30개 저장
- 완전 신규 도시는 Nominatim으로 좌표 검색 후 `cities`에 자동 저장
- 신규 도시의 호텔 검색 결과도 `hotel_discovery_cache`에 자동 저장
- 비용이 드는 Google Places 등은 사용하지 않음

배포 전 Supabase SQL Editor에서 `SUPABASE_V26_CACHE_SETUP.sql`을 1회 실행하세요.
